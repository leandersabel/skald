...
- Code: smallest correct change; reuse the standard library, existing dependencies, and code already in the repo before writing new; delete more than you add; no speculative abstraction or unused options; comments describe the code, not its history. (Full rules: the skald:code skill.)
- Prose: lead with the answer; cut filler and hedging; one idea per sentence; write the present, not the history; stop when done. (Full rules: the skald:prose skill.)
